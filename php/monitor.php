...
// Step 6 Daten übernehmen
$cpu = $_POST['cpu'] ?? '';
$ram = $_POST['ram'] ?? '';
$gehaeuse = $_POST['gehaeuse'] ?? '';
$zubehoer = isset($_POST['zubehoer']) && is_array($_POST['zubehoer']) ? $_POST['zubehoer'] : [];
$ausstattung = isset($_POST['ausstattung']) && is_array($_POST['ausstattung']) ? $_POST['ausstattung'] : [];
$os = $_POST['os'] ?? ''; // os ist eine radio Optionsfeld-ID, kein Array
$software = isset($_POST['software']) && is_array($_POST['software']) ? $_POST['software'] : [];

// Filterwert aus der URL auslesen (z.B. ?filter=22), leer wenn nicht gesetzt
$filter = $_GET['filter'] ?? '';

// Monitor-Daten abfragen
...
<main class="container py-5">
  ...
  <form action="zusammenfassung.php" method="post">
    <!-- hidden fields -->
    ...
    <?php foreach ($software as $s): ?>
      <input type="hidden" name="software[]" value="<?= htmlspecialchars($s) ?>">
    <?php endforeach; ?>
      ...
      <?php while ($row = $monitore->fetch_assoc()): ?>
        <div class="col-md-3 mb-4">
          <div class="card h-100">
            <img src="../img/<?= $row['bildpfad_thumb'] ?>"
                 class="card-img-top preview img-fluid img-thumbnail"
                 onclick="enlarge('../img/<?= $row['bildpfad_gross'] ?>')"
            ...
            <div class="card-body">
              ...
              <div class="form-check">
                <input class="form-check-input" type="checkbox" name="monitore[]" value="<?= $row['id'] ?>" id="m<?= $row['id'] ?>">
